...
import { secureRegister } from "@/services/api";
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      Toast.show({
        ...
        text2: result.message || "Hesabınız başarıyla oluşturuldu! ��",
        position: "top",
      });
      ...
      if (error.response?.status === 400) {
        errorMessage = error.response.data?.error || "Geçersiz veri girişi. Lütfen bilgilerinizi kontrol edin.";
      } else if (error.response?.status === 409) {
        errorMessage = "Bu telefon numarası veya e-posta adresi zaten kayıtlı.";
      } else if (error.response?.status === 429) {
        errorMessage = "Çok fazla kayıt denemesi. Lütfen biraz bekleyip tekrar deneyin.";
